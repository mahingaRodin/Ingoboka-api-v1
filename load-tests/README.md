...
# Override target or credentials via params / env (never commit real values)
$env:BASE_URL = "https://staging.example.com/api/v1"
$env:EMAIL = $env:LOAD_TEST_EMAIL      # or set in load-tests/.env
$env:PASSWORD = $env:LOAD_TEST_PASSWORD
.\load-tests\scripts\run-smoke.ps1
...
```powershell
cd load-tests
# Ensure load-tests/.env exists (from .env.example) or export EMAIL/PASSWORD
